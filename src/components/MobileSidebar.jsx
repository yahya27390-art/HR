          ...
          {groups.map((grp, gIdx) => {
            ...
            return (
              <div key={gIdx} className="space-y-2">
                <div className="px-2 py-1 flex items-center justify-between text-xs font-black text-slate-200">
                  <span className="flex items-center gap-2">
                    <span 
                      className="w-3 h-3 rounded-full shadow-md transition-transform"
                      style={{ 
                        backgroundColor: grp.color || '#10b981',
                        boxShadow: `0 0 10px ${grp.color || '#10b981'}80`
                      }}
                    />
                    <span className="font-heading font-black text-white text-xs">{grp.label || grp.group || "القائمة"}</span>
                    {grp.sublabel && (
                      <span className="text-[10px] text-slate-400 font-medium">({grp.sublabel})</span>
                    )}
                  </span>
                  <span 
                    className="text-[10px] font-mono px-2 py-0.5 rounded-full font-black border"
                    style={{ 
                      borderColor: `${grp.color || '#10b981'}40`,
                      backgroundColor: `${grp.color || '#10b981'}15`,
                      color: grp.color || '#10b981'
                    }}
                  >
                    {grp.items.length}
                  </span>
                  ...
                  {grp.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        to={item.to}
                        onClick={onClose}
                        className={`flex items-center justify-between px-3.5 py-2.5 rounded-2xl text-xs font-bold transition-all duration-200 border ${
                          active
                            ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-900/40 border-emerald-400/50 scale-[1.01]'
                            : 'bg-slate-900/70 hover:bg-slate-800/90 text-slate-200 hover:text-white border-slate-800/80 hover:border-slate-700'
                        }`}
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div 
                            className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-all ${
                              active ? 'bg-white/20 text-white shadow-inner' : 'bg-slate-800/90'
                            }`}
                            style={{
                              color: active ? '#FFFFFF' : (grp.color || '#10b981'),
                              backgroundColor: active ? 'rgba(255, 255, 255, 0.2)' : `${grp.color || '#10b981'}18`
                            }}
                          >
