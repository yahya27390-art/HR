          ...
          {groups.map((grp, gIdx) => {
            ...
            return (
              <div key={gIdx} className="space-y-1.5">
                <div className="px-2.5 py-1 flex items-center justify-between text-xs font-bold text-slate-200">
                  <span className="flex items-center gap-2">
                    <span 
                      className="w-2.5 h-2.5 rounded-full shadow-sm"
                      style={{ backgroundColor: grp.color || '#10b981' }}
                    />
                    <span className="font-heading font-black">{grp.label || grp.group || "القائمة"}</span>
                    {grp.sublabel && (
                      <span className="text-[10px] text-slate-400 font-normal">({grp.sublabel})</span>
                    )}
                  </span>
                  <span className="text-[10px] text-slate-400 font-mono px-2 py-0.5 rounded-full bg-slate-800/80 border border-slate-700/60 font-bold">
                    {grp.items.length}
                  </span>
                </div>
                  ...
                  {grp.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <div 
                            className={`w-7 h-7 rounded-xl flex items-center justify-center shrink-0 ${
                              active ? 'bg-white/20 text-white' : 'bg-slate-800/80'
                            }`}
                            style={{
                        ...
                        {active && <ChevronLeft className="w-4 h-4 shrink-0" />}
                      </Link>
                    );
                  })}
                </div>
